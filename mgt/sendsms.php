...
if(isset($_POST['sendmsg'])){
	...
	if(empty($_POST['contacts'])){
		...
		foreach($_POST['contacts'] as  $key => $val){
			...
			$sql="SELECT * FROM `attend` WHERE id='".$_POST['contacts'][$key]."'";
			$results=$connect->retrieve($sql);
			$rows=$connect->arrayFetch($results);
			foreach($rows as $row){
				
				 $response=$connect->CURLsendsmsresend($row['phone'],$row['name'],$_POST['message']);
				

         $query="INSERT INTO `sentmsg` (`id`,`name`,`phone`,`msg`,`status`,`day`,`time`)
		 VALUES (NULL,'".filter_var($row['name'], FILTER_SANITIZE_STRING)."','".filter_var($row['phone'], FILTER_SANITIZE_STRING)."','".filter_var($_POST['message'], FILTER_SANITIZE_STRING)."','".$response."','".date('d-m-Y')."','".date('H:i:s')."')";
